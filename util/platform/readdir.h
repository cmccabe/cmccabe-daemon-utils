...
#ifndef DUTILS_UTIL_PLATFORM_READDIR_DOT_H
#define DUTILS_UTIL_PLATFORM_READDIR_DOT_H
 ...
 */
int  do_opendir(const char *name, struct util_dirp** dp);
 ...
 */
struct dirent *do_readdir(struct util_dirp *dp);

/** Close a directory and free the associated resources
 *
 * @param dp		directory to close
 */
void do_closedir(struct util_dirp *dp);
